...
games = games.drop(['school_score','opp_score','ot','attend'], axis = 1)

school_geo = pd.read_csv('geo\\csv\\school_coord.csv', header = 0)
opp_geo = school_geo.copy().rename(index = str, columns = {'school_id': 'opp_id'})
offcampus_geo = pd.read_csv('geo\\csv\\neutral_coord.csv', header = 0)
...
with open('geo\\csv\\game_dist.csv', 'wb') as csvfile:
