...
games <- lmer_df[is.na(lmer_df$efg) == F,]
...
formula <- efg ~ (1|off_sid) + (1|def_sid) + (1|location) + (1|ot) + (1|off_dist_gr) + (1|def_dist_gr)
...
for (season in 2009:2017){
  ...
  colnames(ran) <- c('school_sid','off_efg','def_efg')
  
  efg_df <- rbind(efg_df,ran)
}