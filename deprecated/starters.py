...
game_hrefs = pd.read_csv('ncaa_scrapers\\csv\\game_hrefs.csv', header = 0)

try:
    starters = pd.read_csv('ncaa_scrapers\\csv\\starters.csv', header = 0)
except IOError as error:
    if str(error) == 'File ncaa_scrapers\csv\starters.csv does not exist':
        with open('ncaa_scrapers\\csv\\starters.csv', 'wb') as csvfile:
...
for game_href in game_hrefs_needed:
    ...
    for period in [1,2]:
        ...
        try:
            ...
            with open('ncaa_scrapers\\csv\\starters.csv', 'ab') as hrefscsv:
