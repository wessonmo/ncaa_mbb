...
games <- lmer_df[is.na(lmer_df$pta_min) == F,]
...
formula <- pta_min ~ (1|off_sid) + (1|def_sid) + location + (1|ot) + (1|off_dist_gr) + (1|def_dist_gr)
...
for (season in 2009:2017){
  ...
  colnames(ran) <- c('school_sid','off_ptam','def_ptam')
  
  pta_df <- rbind(pta_df,ran)
}
