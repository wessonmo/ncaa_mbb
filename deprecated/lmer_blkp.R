...
games <- lmer_df[is.na(lmer_df$blkp) == F,]
...
games$blkp_log <- log(games$blkp + 0.01)

formula <- blkp_log ~ (1|off_sid) + (1|def_sid) + (1|location) + (1|off_dist_gr) + (1|def_dist_gr) + (1|ot)
...
for (season in 2009:2017){
  ...
  colnames(ran) <- c('school_sid','off_blkp','def_blkp')
  
  blkp_df <- rbind(blkp_df,ran)
}