...
games <- lmer_df[is.na(lmer_df$orbp) == F,]
...
formula <- orbp ~ (1|off_sid) + (1|def_sid) + off_dist_gr + def_dist_gr + (1|location)
...
for (season in 2009:2017){
  ...
  colnames(ran) <- c('school_sid','off_orbp','def_orbp')
  
  orbp_df <- rbind(orbp_df,ran)
}