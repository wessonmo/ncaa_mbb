...
#        print('Facilities, Coaches, and Schedules: {0}% Complete'.format(percent_complete), end = '\r')
        ...
        if row.schedules == 'left_only':
            ...
            sched['opp_id'] = [int(re.compile('[0-9]+').search(y.get('href')).group(0)) if y else None
                 for y in [x[0].find('a') for x in games]]
            sched['game_date'] = [x[1].text for x in games]
            sched['school_score'] = [int(x[2].text) for x in games]
            sched['opp_score'] = [int(x[3].text) for x in games]
            sched['game_type'] = [x[4].text.strip() for x in games]
            sched['location'] = [x[5].text.strip() if x[5].text else None for x in games]
            sched['ot'] = [int(x[6].text.strip().split(' ')[0]) if '-' not in x[6].text else None for x in games]
            sched['attend'] = [int(re.sub(',','',x[7].text.strip())) for x in games]
