...
all_game_ids = pd.read_csv('ncaa_scrapers\\csv\\game_ids.csv', header = 0)
all_game_ids = set(x for x in set(all_game_ids.loc[pd.isnull(all_game_ids.game_id) == False].game_id))

try:
    scraped_game_ids = set(pd.read_csv('ncaa_scrapers\\csv\\box_scores.csv', header = 0).game_id)
except IOError as error:
    if str(error) == 'File ncaa_scrapers\\csv\\box_scores.csv does not exist':
        with open('ncaa_scrapers\\csv\\box_scores.csv', 'wb') as csvfile:
            csvwriter = csv.writer(csvfile, delimiter = ',', quotechar = '"', quoting = csv.QUOTE_MINIMAL)
            csvwriter.writerow(['game_id','period','school_id','player_order','player_href','player_name','pos','min'] + box_score_var)
...
for game_id in game_ids_needed:
    for period in [1,2]:
        ...
        try:
            ...
            with open('ncaa_scrapers\\csv\\box_scores.csv', 'ab') as hrefscsv:
        ...
        for box_score_soup, school_id in zip(box_scores, school_ids):
            ...
            data['game_href'] = [game_id]*len(players)
